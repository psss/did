...
class Pagure():
    ...
    def __init__(self, url: str, token: str | None, timeout: int = TIMEOUT):
...
class Issue():
    ...
    def __init__(self, data: dict[str, Any], options):
        ...
        try:
            self.closed: datetime.date | None = datetime.datetime.fromtimestamp(
                float(data['closed_at'])).date()
        except TypeError:
            self.closed = None
        try:
            self.closed_by: str | None = data["closed_by"]["name"]
        except TypeError:
            self.closed_by = None
