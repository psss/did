...
def test_statsgroup_class(capsys):
...
def test_userstats_missing_type():
...
def test_userstats_unknown_type():
...
def test_userstats_invalid_order():
...
def test_userstats_class(capsys):
    config = did.base.Config(EMPTY_CONFIG)
...
def test_emptystats_class(capsys):
...
def test_emptystatsgroup_class(capsys):
